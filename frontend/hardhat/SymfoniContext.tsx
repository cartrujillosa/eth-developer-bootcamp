...
import ItemManagerDeployment from "./../../hardhat/deployments/localhost/ItemManager.json";
import { ItemManager } from ".//ItemManager";
import { ItemManager__factory } from "./factories/ItemManager__factory";
import { Payment } from ".//Payment";
import { Payment__factory } from "./factories/Payment__factory";
...
export const ItemManagerContext = React.createContext<SymfoniItemManager>(emptyContract);
export const PaymentContext = React.createContext<SymfoniPayment>(emptyContract);
...
export interface SymfoniItemManager {
    instance?: ItemManager;
    factory?: ItemManager__factory;
}

export interface SymfoniPayment {
    instance?: Payment;
    factory?: Payment__factory;
}
...
export const Symfoni: React.FC<SymfoniProps> = ({
    ...
    const [ItemManager, setItemManager] = useState<SymfoniItemManager>(emptyContract);
    const [Payment, setPayment] = useState<SymfoniPayment>(emptyContract);
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        const doAsync = async () => {
            ...
            const finishWithContracts = (text: string) => {
                setItemManager(getItemManager(_provider, _signer))
                setPayment(getPayment(_provider, _signer))
                finish(text)
            }
    ...
    const getItemManager = (_provider: providers.Provider, _signer?: Signer) => {

        const contractAddress = ItemManagerDeployment.receipt.contractAddress
        const instance = _signer ? ItemManager__factory.connect(contractAddress, _signer) : ItemManager__factory.connect(contractAddress, _provider)
        const contract: SymfoniItemManager = {
            instance: instance,
            factory: _signer ? new ItemManager__factory(_signer) : undefined,
        }
        return contract
    }
        ;
    const getPayment = (_provider: providers.Provider, _signer?: Signer) => {
        let instance = _signer ? Payment__factory.connect(ethers.constants.AddressZero, _signer) : Payment__factory.connect(ethers.constants.AddressZero, _provider)
        const contract: SymfoniPayment = {
            instance: instance,
            factory: _signer ? new Payment__factory(_signer) : undefined,
        }
        return contract
    }
        ;
            ...
            <ProviderContext.Provider value={[provider, setProvider]}>
                <SignerContext.Provider value={[signer, setSigner]}>
                    <CurrentAddressContext.Provider value={[currentAddress, setCurrentAddress]}>
                        <ItemManagerContext.Provider value={ItemManager}>
